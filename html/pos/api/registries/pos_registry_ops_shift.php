...
function handle_shift_end(PDO $pdo, array $config, array $input_data): void {
    ...
    // [L4-2B-POS-EOD-MINI] 依赖: compute_expected_cash + getInvoiceSummaryForPeriod (来自 pos_repo.php)
    ...
    // [L4-2B-POS-EOD-MINI] 获取支付方式分解，用于写入 pos_shifts.payment_summary
    // 此处与强制关班逻辑对齐，确保正常班次与强制班次的 pos_shifts 字段完整度一致
    $invoice_summary = getInvoiceSummaryForPeriod($pdo, $store_id, $shift['start_time'], $now_utc_str);
    $payment_breakdown = $invoice_summary['payments'] ?? ['Cash' => 0.0, 'Card' => 0.0, 'Platform' => 0.0];
    $payment_summary_json = json_encode($payment_breakdown);

    // [L4-2B-POS-EOD-MINI] 正常关班现在写入完整的财务字段（expected_cash, cash_variance, payment_summary）
    // 这与强制关班保持一致，避免同一张表中字段完整度不同的问题
    ...
    $upd->execute([$now_utc_str, $counted_cash, $expected_cash, $cash_diff, $payment_summary_json, $shift_id]);
...
function handle_shift_force_start(PDO $pdo, array $config, array $input_data): void {
    ...
    foreach ($ghosts as $ghost) {
        ...
        // [L4-2B-POS-EOD-MINI] 获取支付方式分解，用于写入 pos_shifts.payment_summary
        // 此处与正常关班逻辑对齐，确保所有班次的 pos_shifts 字段完整度一致
        $invoice_summary = getInvoiceSummaryForPeriod($pdo, $store_id, $ghost['start_time'], $now_utc_str);
        $payment_breakdown = $invoice_summary['payments'] ?? ['Cash' => 0.0, 'Card' => 0.0, 'Platform' => 0.0];
        // 在强制关班场景下，保留管理员信息作为附加字段
        $payment_breakdown['_force_closed_by'] = $closer_name;
        $payment_breakdown['_force_closed_note'] = 'Shift was forcibly closed due to EOD cutoff';
        $payment_summary_json = json_encode($payment_breakdown);
